...
!!! warning "Keep the attribute keys consistent"
    The `*_attribute` keys describe one schema written by `RegisterConfig` and read by both
    `DiscoveryConfig` and `HealthConfig`. Override a key in one place and you must override it in
    all three — otherwise the balancer discovers nothing, or the health daemon probes nothing.
